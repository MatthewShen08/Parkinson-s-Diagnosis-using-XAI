...
hc_directory = "C:/Users/mtshe/Downloads/HC_AH/HC_AH"
pd_directory = "C:/Users/mtshe/Downloads/PD_AH/PD_AH"
...
# SHAP Explainability for RandomForest Model using shap.Explainer
explainer_rf = shap.Explainer(rf_model, X_train_selected)  # Use shap.Explainer
shap_values_rf = explainer_rf(X_test_selected)

# Plot SHAP Summary for RandomForest
plt.figure(figsize=(12, 8))
shap.summary_plot(shap_values_rf, X_test_selected, feature_names=full_df.drop(columns=['label', 'file']).columns)

# SHAP Explainability for GRU Model using shap.Explainer
explainer_gru = shap.Explainer(keras_gru_model.model_, X_train_selected.reshape(-1, 1, X_train_selected.shape[1]))
shap_values_gru = explainer_gru(X_test_selected.reshape(-1, 1, X_test_selected.shape[1]))

# Plot SHAP Summary for GRU Model
plt.figure(figsize=(12, 8))
shap.summary_plot(shap_values_gru, X_test_selected, feature_names=full_df.drop(columns=['label', 'file']).columns)
